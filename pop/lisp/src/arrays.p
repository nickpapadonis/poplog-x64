...
define lconstant Checkr_bounds(dims) -> (bounds, size, rank);
	...
	[% for dim in_cl_list dims do
		...
		endunless;
		dim * size -> size;         /* ____size might be a biginteger */
		rank fi_+ 1 -> rank;
		0, dim fi_- 1
	endfor %] -> bounds;
...
define make_array(dims, type, init, contents, adjust, fp, disp, offset) -> a;
	...
	if adjust /== nil then
		...
		if disp /== nil then
			...
			if adjust == @:DISPLACED then   /* i.e. ____disp is adjustable */
...
define lconstant Do_bits(a1, a2, a3, bit_op) -> a3;
	...
	/* Can use fast_subscrs for processing bit-vectors if start indexes
		___lo1, ___lo2, and ___lo3 are all byte-aligned
	*/
	if  (lo1 fi_// 8 -> lq1) == 1
	and (lo2 fi_// 8 -> lq2) == 1
	and (lo3 fi_// 8 -> lq3) == 1
	...
	/* Apply ______bit_op to each (remaining) pair of bits */
...
define bit_not(a1, a2) -> a2;
	...
	/* Can use fast_subscrs if start indexes ___lo1 and ___lo2 are byte-aligned */
...
--- John Williams, Dec 10 1993
		lconstant __________Isbitarray renamed as permanent identifier isbitarray.
