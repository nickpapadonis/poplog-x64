...
;;; Makes use of sys_timer, available since Poplog V14.02
...
define lconstant copy_from_disk(newfile);
	;;; make backup copy of current file  to newfile if original exists on disk
	;;; if not, write it first
	lvars newfile;
	unless sys_file_stat(vedpathname, nullvector ) then
		ved_w1()
	endunless;
	sys_file_copy(vedpathname, newfile) ;;; uses pop_file_versions
enddefine;
...
define lconstant do_autosave_backup();
	...
	if isinteger(vedautosave_preserve) then
		;;; make sure that enough hyphens are appended the first time, to survive
		;;; subsequent saves in the same session.
		max(vedautosave_preserve, pop_file_versions + 1)
						-> pop_file_versions ;;; local to vedautosave_write
	elseif isstring(vedautosave_preserve) then
		;;; URGH! on Unix '~' gets interpreted as user directory. So to rule that out
		;;; prepend current directory before testing if it's a directory.
		copy_from_disk(
			if
	...
	elseif isprocedure(vedautosave_preserve) then
		...
		;;; Default: if true simply use an extra version
		copy_from_disk(vedpathname sys_>< backup_string(pop_file_versions + 1))
	endif
enddefine;
...
define global vedset_autosave(set);
	...
	define lconstant check_autosave;
		...
		dlocal
			autosaving_now = true,
			in_a_read =
				vedprintingdone or iscaller(vedrestorescreen) or iscaller(charin);
	...
	if set then
		sys_real_time(), round(autosave_mins * 60e6)
	else
		false, false
	endif -> (last_time_run, sys_timer(check_autosave))
