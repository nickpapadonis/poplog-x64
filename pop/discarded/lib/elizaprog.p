...
define readsentence()->sentence;
	lvars item char sentenceread;
	dlocal proglist popprompt sentence;
	define repeater();
		cucharin() -> char;
...
define gtrl;
	...
	applist(list, sysSYNTAX(%0, false%));
	[newrule ^Name ;
		dlocal ^^list;
...
newrule youarenot;
	dlocal L1;
	if itmatches([you are not ??L1]) then
		oneof([['would you be happier if you were' ^^L1]
			['Perhaps you are lucky not to be' ^^L1]])
	endif
endnewrule;

newrule notsomething;
	dlocal L1;
	if itmatches([not ??L1]) then
		oneof([['why not' ^^L1]
				['Do you have negative feelings about' ^^L1]])
	endif
endnewrule;

vars earlycount, list;
...
define eliza();
	dlocal problem sentence inchar cucharin earlycount;
...
/*  --- Revision History ---------------------------------------------------
--- John Gibson, Jul  7 1995
		Made it compile under 14.53
