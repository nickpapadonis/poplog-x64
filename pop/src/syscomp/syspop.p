...
define lconstant make_record_struct(speclist, create_substr, pop);
	...
	for spec in speclist do
		...
		elseif spec == "|" then
			;;; end of record in 'union' (only when ___pop false)
			max(bitoffs, totbitoffs) -> totbitoffs;
			0 -> bitoffs
		else
			;;; adds fields to __________field_list
...
#_ENDIF
	...
	if isref(spec) then
		;;; access of external pointer value (_______isfield and ______impacc must be false)
...
define match_struct_init(init, struct_spec, base_p, pop, popacc, top)
	...
	fast_for field_spec in spec_list do
		...
		if field_spec(FIELD_IDENT) ->> fid then
			;;; explicit field
			if iskey(pop) then
				;;; doing pop recordclass -- ______popacc is the vector of access
				;;; procedures
				if fid == "KEY" then
					pop
				else
...
define lconstant read_extern_name() -> name;
	...
	if nextreaditem() == "/" then
		;;; ____name is language name prefix
		readitem() -> ;
