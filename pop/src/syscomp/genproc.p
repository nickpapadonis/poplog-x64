...
;;; wkreg:
;;;		<true> if ___opd is a work register
...
;;;     _________regnumber is used as a recogniser for legal register names. NB:
...
;;;     where ____base and _____index are registers, ____disp is an immediate
;;;     displacement and _____scale is an index scale factor (1,2,4 or 8).
;;;     The _____index and _____scale components are usually omitted, and are
...
;;;     If ___all is <false>, then only the work registers are flushed; this
;;;     is done after every instruction.
;;;     If ___all is <true>, then the user stack offset is flushed too; this
...
;;; adjust:
;;; 	adjust an M-code operand to remove auto-indirection; _____delta is the
...
;;;		___asm___op determines the type of the source operand and whether
...
;;; gen_lea:
;;;		compute an address from operands ____src1 and ____src2 and load it
;;;		to ___dst. Both sources must be register or immediate operands.
...
;;;			dst := op(src)
;;;     The operator will be either NEG or NOT, determined by ___asm___op.
;;;     Both of these instructions take only a single operand, so if ___dst
;;;     is not the same as ___src, an extra move is required.
...
;;;		___asm___op. All these instructions take only two operands,
;;;		one of which must not be in memory.
;;;		If the operation is ADD and _________set_flags is true, then the code
...
define lconstant gen_op_2(src1, src2, dst, asm_op);
	...
	/* {____src1 __/= ___dst _| ____src2 _= ___dst} */
	if src2 = dst then
...
;;;		plants code for the operation
;;;			dst := src2 * src1
;;;		This is different from gen_op_2 because IMUL requires ___dst to be
...
define lconstant gen_mult(src1, src2, dst);
	...
	/* {__________~imm(src1) _| __________~imm(src2)} */
	if ismem(src1) then (src1, src2) -> (src2, src1) endif;
	/* {__________~mem(src1) _| _________mem(src2)} */
	if isreg(dst)
	...
	elseif ismem(src1) then
		/* {_________mem(src1) _& _________mem(src2) _& ___tmp __/= ____________________reg_in_operand(src2)} */
		asmMOVL(src1, tmp);
		asmIMULL(src2, tmp, tmp);
	else
		/* {_________reg(src1) _& ____src1 __/= ___tmp} */
		asmMOVL(src2, tmp);
...
;;;			dst := src2 shift src1
;;;		where ____src1 will always be an immediate quantity.
...
;;;		on machine integers; _____gen_p is the code-planting instruction for
...
;;;     directly. The ____type field of the instruction is ignored.
...
;;; M_BIC:
;;;     clears the bits in ____src2 which are set in ____src1 and places the
;;;     result in ___dst, i.e. performs the operation
;;;         dst := src2 && ~~src1
...
;;; M_ASH:
;;;     performs an arithmetic shift of ____src2 by an amount ____src1, leaving
;;;     result in ___dst. The shift may be right or left depending on the
;;;     sign of ____src1.
...
;;; gen_test_or_cmp:
;;;     plants code to test/compare ____src1 against ____src2, and jump to ___lab
;;;     on ____test. The test may be CMP (src1 - src2) or TEST (src1 &&
;;;     src2) determined by _____gen_p; the test will be one of the M-code
...
;;;             printf('loading big constant to register\n');
	...
	if isimm(src1) or ismem(src2) then
		...
		;;; (faster) to have it as ____src1
		(src1, src2) -> (src2, src1);
...
define M_CMPKEY();
	...
	;;; move ___src to a register to allow for accessing its key
...
;;; gen_switch:
;;;     plants a computed goto on the integer ___src: this may be a system
;;;     integer or a POP integer depending on the flag ______sysint.
;;;     ____labs is a list of the labels to jump to, counted from 1.
...
define lconstant gen_switch(src, labs, else_case, sysint);
	...
	;;; whether ___src is a sysint: if so, we want (table_start + src * 4),
...
;;; M_BRANCH <label>:
;;;     unconditional jump to _____label
...
;;;     unconditional jump to execute the procedure ___opd. Such an operand
;;;     has an extra level of indirection over that already present in ___opd.
;;;     If ___opd is a POP procedure, the computed operand must refer to its
;;;     execute address.
...
define mc_code_generator(codelist, hdr_len) -> (gencode, pdr_len);
	...
	define lconstant gencode();
		...
		;;; Define _______pdr_len as the size in words of the procedure
